...
def generate_response(
    ...
    try:
        ...
        if intent == "explain" and (chat_context or diagnosis):
            system_prompt = _build_explain_prompt(chat_context, diagnosis)
        elif intent == "onboard" and (chat_context or diagnosis):
            system_prompt = _build_onboard_prompt(chat_context, diagnosis, candidate_issues)
        else:
            system_prompt = _build_chat_prompt(chat_context, diagnosis)
        ...
        return _generate_fallback(intent, message, chat_context, diagnosis, candidate_issues)
...
def _generate_fallback(
    ...
    if any(kw in msg_lower for kw in ["기여", "초보자", "시작", "어떻게"]):
        ...
        return "오픈소스 기여를 시작하려면 먼저 저장소를 분석해주세요."
