...
def build_supervisor_graph_async() -> StateGraph:
    """비동기 노드를 사용하는 Supervisor 그래프 빌드."""
    from backend.agents.supervisor.nodes.meta_nodes import (
    ...
    from backend.agents.supervisor.nodes.reflection_nodes import (
        self_reflection_node_async,
    )
    
    graph = StateGraph(SupervisorState)

    # 비동기 노드 등록 - 메타 에이전트
    graph.add_node("parse_supervisor_intent", parse_supervisor_intent_async)
    graph.add_node("create_supervisor_plan", create_supervisor_plan)  # 동기 (LLM 호출 없음)
    graph.add_node("execute_supervisor_plan", execute_supervisor_plan)  # 동기 (내부에서 별도 호출)
    graph.add_node("reflect_supervisor", reflect_supervisor_async)
    graph.add_node("finalize_supervisor_answer", finalize_supervisor_answer_async)

    # 기존 노드들 (LLM 호출 없는 노드들은 동기 유지)
    graph.add_node("intent_analysis_node", intent_analysis_node)
    graph.add_node("decision_node", decision_node)
    graph.add_node("run_diagnosis_node", run_diagnosis_node)
    graph.add_node("quality_check_node", quality_check_node)
    ...
    graph.add_node("smart_error_recovery_node", smart_error_recovery_node)
    graph.add_node("partial_result_recovery_node", partial_result_recovery_node)
    graph.add_node("self_reflection_node", self_reflection_node_async)
    ...
    def _route_after_error_recovery(state: SupervisorState) -> Literal[
        ...
        if next_node == "run_diagnosis_node":
            return "run_diagnosis_node"
        elif next_node == "partial_result_recovery_node":
            return "partial_result_recovery_node"
        return "__end__"
    ...
    graph.add_edge("partial_result_recovery_node", "quality_check_node")

    def _route_after_quality(state: SupervisorState) -> Literal[
        "run_diagnosis_node", "fetch_issues_node", "compare_results_node", "self_reflection_node", "__end__"
    ]:
...
def get_supervisor_graph_async():
    """비동기 노드를 사용하는 컴파일된 Supervisor 그래프 반환."""
    graph = build_supervisor_graph_async()
    
    from langgraph.checkpoint.memory import MemorySaver
    checkpointer = MemorySaver()
    logger.info("Using MemorySaver for async state persistence.")
