...
class SecurityAgentV2:
    ...
    def _build_graph(self) -> StateGraph:
        """LangGraph 구조 생성"""
        workflow = StateGraph(SecurityAnalysisStateV2)
    ...
    def _after_reflection(self, state: SecurityAnalysisStateV2) -> str:
        """반성 후 다음 액션 결정"""
        # 전략 변경이 있었다면 재계획
    ...
    async def analyze(
        ...
        # 초기 상태 생성
        initial_state = create_initial_state_v2(
        ...
        try:
            ...
            print(f"\n[SecurityAgentV2] Error during execution: {e}")
            return {
                "success": False,
                "error": str(e),
                "partial_results": {
    ...
    def get_conversation_history(self, state: SecurityAnalysisStateV2) -> str:
        """대화 히스토리 가져오기"""
    ...
    def export_state(self, state: SecurityAnalysisStateV2, format: str = "json") -> str:
        """상태 내보내기"""
        if format == "json":
...
async def quick_analysis(
    ...
    """
    agent = SecurityAgentV2(execution_mode=mode)
