...
- **Lock 파일 우선 처리**: Lock 파일에서 추출된 정확한 버전 정보 우선 사용
- 의존성 통계 및 요약 제공
- 보안 점수 계산 (기본 구현)
- 보안 개선 사항 제안

**Lock 파일 지원:**
Lock 파일은 실제로 설치된 패키지의 정확한 버전을 기록하는 파일로, 재현 가능한 빌드를 보장합니다. 시스템은 lock 파일을 자동으로 감지하고, lock 파일의 의존성을 일반 파일보다 우선적으로 사용합니다.

지원하는 주요 lock 파일:
- `package-lock.json`, `yarn.lock` (JavaScript/Node.js)
...
├── config/
│   └── dependency_files.py         # 지원하는 의존성 파일 및 Lock 파일 패턴 정의
├── models/
...
- `is_from_lockfile` (bool): Lock 파일에서 추출되었는지 여부 (기본값: False)
...
# Lock 파일의 의존성 (정확한 버전)
dep_lock = Dependency(name="requests", version="2.28.1", type="runtime", source="pypi", is_from_lockfile=True)
```
...
1. 파일명 추출
2. LOCK_FILES 패턴과 비교
3. Lock 파일로 확인되면 True 반환

**사용 시점:** 의존성 추출 시 lock 파일 여부를 판단하여 정확한 버전 정보 우선 처리
...
4. **중복 제거 (Lock 파일 우선)**:
   - `source:name`을 키로 사용
   - **Lock 파일의 의존성을 우선적으로 사용** (정확한 버전 정보)
   - Lock 파일이 아닌 경우, 버전이 있는 의존성 우선
5. Lock 파일에서 추출된 의존성 개수 집계
6. 결과 딕셔너리 구성 및 반환 (Lock 파일 통계 포함)
...
[7] ThreadPoolExecutor (병렬 처리 시작, max_workers=5)
    ...
    │           OUTPUT: "{ \"dependencies\": {...} }"
    │   ↓
    │   [8.5] RepositoryAnalyzer.is_lockfile(path)  # Lock 파일 여부 확인
    │       INPUT: path="package.json"
    │       OUTPUT: False (일반 파일)
    │   ↓
    │   [9] DependencyExtractor.extract(content, filename, is_lockfile=False)
    │       │   INPUT: content="{ \"dependencies\": {...} }", filename="package.json", is_lockfile=False
    │       ↓
    │       JavaScriptExtractor.extract(content, filename, is_lockfile=False)
    │           ↓
    │           JavaScriptExtractor._extract_package_json(content)
    │               INPUT: content (JSON 문자열)
    │               OUTPUT: [
    │                   Dependency(name="react-dom", version="^18.0.0", type="runtime", source="npm", is_from_lockfile=False),
    │                   Dependency(name="jest", version="^29.0.0", type="dev", source="npm", is_from_lockfile=False),
    │                   ...
    │               ]
    │
    ├─ [Thread 2] ... package-lock.json 처리 (Lock 파일)
    │   ↓
    │   [8.5] RepositoryAnalyzer.is_lockfile("package-lock.json")
    │       OUTPUT: True (Lock 파일!)
    │   ↓
    │   [9] DependencyExtractor.extract(content, "package-lock.json", is_lockfile=True)
    │       OUTPUT: [
    │           Dependency(name="react-dom", version="18.2.0", type="runtime", source="npm", is_from_lockfile=True),
    │           # 정확한 버전 (^18.0.0 → 18.2.0)
    │           ...
    │       ]
    │
...
[10] RepositoryAnalyzer._build_result(owner, repo, analyzed_files)
    ...
    │   - Lock 파일 개수 카운트
    │   - **중복 제거 (Lock 파일 우선)**:
    │       * source:name 기준으로 중복 확인
    │       * Lock 파일(is_from_lockfile=True) 의존성 우선 선택
    │       * 예: package.json의 "^18.0.0" vs package-lock.json의 "18.2.0"
    │            → Lock 파일의 "18.2.0" 선택 (정확한 버전)
    │   - 소스별 통계 계산
    │   - Runtime/Dev 의존성 집계
    │   - Lock 파일 의존성 개수 집계
    │
    ...
    │           {
    │               'name': 'react-dom',
    │               'version': '18.2.0',  # Lock 파일의 정확한 버전
    │               'is_from_lockfile': True,
    │               ...
    │           }
    │       ],
    │       'summary': {
    ...
    │           'dev_dependencies': 40,
    │           'total_unique': 120,
    │           'lockfile_dependencies': 95,  # Lock 파일 의존성 수
    │           'non_lockfile_dependencies': 25  # 일반 파일 의존성 수
    │       }
    │   }
    ↓
...
Lock 파일은 **실제로 설치된 패키지들의 정확한 버전을 기록**하는 파일입니다:

**주요 특징:**
1. **정확한 버전 기록**: `^1.0.0` 같은 범위 대신 `1.0.5` 같은 정확한 버전 저장
2. **재현 가능한 빌드**: 동일한 환경을 다른 시스템에서 재현 가능
3. **간접 의존성 포함**: 직접 의존성뿐만 아니라 하위 의존성도 모두 기록
4. **자동 생성**: 패키지 설치 시 자동으로 생성/업데이트

**예시:**
```
package.json:         "react": "^18.0.0"  (범위)
package-lock.json:    "react": "18.2.0"   (정확한 버전)
```

### 시스템의 Lock 파일 처리

**1. 자동 감지**
- `LOCK_FILES` 리스트에 정의된 30+ 파일 패턴
- 파일 분석 시 자동으로 lock 파일 여부 확인

**2. 우선 처리**
```python
# analyzer.py:207-221
if dep.is_from_lockfile and not existing.is_from_lockfile:
    unique_dependencies[key] = dep  # Lock 파일 의존성 우선!
```

**3. 정확한 버전 보장**
- Lock 파일: `requests==2.28.1` (설치된 정확한 버전)
- 일반 파일: `requests>=2.28.0` (범위)
- → 시스템은 `2.28.1` 선택 (실제 사용 버전)
...
Lock 파일 정보가 결과에 포함됩니다:

```python
{
    'lock_files_count': 2,  # 발견된 Lock 파일 수
    'files': [
        {
            'path': 'package-lock.json',
            'is_lockfile': True,  # Lock 파일 표시
    ...
    'all_dependencies': [
        {
            'name': 'react',
            'version': '18.2.0',  # 정확한 버전
            'is_from_lockfile': True,  # Lock 파일 출처
            ...
        }
    ],
    'summary': {
        'lockfile_dependencies': 95,  # Lock 파일 의존성
        'non_lockfile_dependencies': 25,  # 일반 파일 의존성
        ...
    }
}
```
...
**1. 보안 분석**
- Lock 파일의 정확한 버전으로 취약점 검사 가능
- 실제 사용 중인 버전 확인

**2. 재현성 검증**
- Lock 파일 존재 여부로 빌드 재현성 평가
- Lock 파일 없으면 경고 제안

**3. 버전 비교**
```python
# 예시: package.json vs package-lock.json 비교
for dep in all_dependencies:
    if dep.is_from_lockfile:
        print(f"{dep.name}: 실제 버전 {dep.version}")
    else:
        print(f"{dep.name}: 범위 {dep.version}")
```
