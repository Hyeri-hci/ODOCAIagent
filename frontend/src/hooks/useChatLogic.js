...
export const useChatLogic = ({
    ...
    const getInitialMessages = useCallback(() => {
        ...
        if (initialAnalysisResult?.summary?.score > 0) initialSections.overview = "complete";
        if (
            initialAnalysisResult?.technicalDetails?.stars > 0 ||
            initialAnalysisResult?.technicalDetails?.forks > 0 ||
        ...
        setMessages((prevMessages) => {
            return prevMessages.map((msg) => {
                if (msg.type === "report_generation") {
                    ...
                    if (analysisResult.technicalDetails) updatedSections.metrics = "complete";
                    if (analysisResult.projectSummary) updatedSections.projectSummary = "complete";
                    if (analysisResult.security || analysisResult.securityRequested) updatedSections.security = "complete";
                    if (analysisResult.risks?.length > 0) updatedSections.risks = "complete";
                    if (analysisResult.recommendedIssues?.length > 0) updatedSections.recommendedIssues = "complete";
                    if (analysisResult.recommendations?.length > 0) updatedSections.recommendations = "complete";
                    if (analysisResult.similarProjects?.length > 0) updatedSections.similarProjects = "complete";
                    if (analysisResult.onboardingPlan?.length > 0) updatedSections.onboardingPlan = "complete";

                    return {
                        ...msg,
                        sections: updatedSections,
                        progressMessage,
                        isComplete: Object.values(updatedSections).filter((s) => s === "complete").length >= 3,
                    };
                }
                return msg;
            });
        });
    }, [analysisResult, showReport, userClosedReport, progressMessage, setMessages]);
    ...
    const fetchAIResponse = async (userMessage) => {
        try {
            ...
            const response = await sendChatMessageV2(userMessage, sessionId, owner, repo);
            ...
            return response.answer || "죄송합니다. 응답을 생성하는데 문제가 발생했습니다.";
        } catch (error) {
    ...
    const handleSendMessage = async () => {
        if (!inputValue.trim() || isAnalyzing || (USE_STREAM_MODE && isStreaming)) return;
        ...
        if (detectedUrl) {
            if (USE_WEBSOCKET) {
                ...
                sendWsAnalysisRequest(detectedUrl, ["diagnosis", "security", "recommend", "contributor", "comparison"]);
                return;
            }
    ...
    const handleSendGuideMessage = (guideMessage, options = {}) => {
        if (options.asUserMessage) {
            addMessage({ id: `user_${Date.now()}`, role: "user", content: guideMessage, timestamp: new Date() });
            setIsTyping(true);
            startStream(guideMessage, sessionId, analysisResult, addMessage, setIsTyping, sessionRepo);
        } else {
            setMessages(prev => [...prev, { id: Date.now(), role: "assistant", type: "guide", content: guideMessage, timestamp: new Date() }]);
        }
    ...
    const handleCompareAnalysis = async () => {
        ...
        if (repositories.length !== 2) {
            addMessage({ id: `err_${Date.now()}`, role: "assistant", content: "비교하려면 2개의 저장소를 선택해주세요.", timestamp: new Date() });
            return;
        }
        setIsComparing(true);
        ...
        try {
            ...
            addMessage({ id: `err_${Date.now()}`, role: "assistant", content: `오류: ${e.message}`, timestamp: new Date() });
        } finally {
